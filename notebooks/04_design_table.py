...
# what is the largest particle radius that keeps the suspension
# `homogeneous`, and the smallest radius that classifies as
# `sedimented`?"*
#
# Two tables result:
#
# 1. **Homogeneous-band upper edge**: largest `r` per (h, t_obs, T)
#    with regime ≡ `homogeneous`. The "stay-mixed" envelope.
# 2. **Sedimented-band lower edge**: smallest `r` per (h, t_obs, T)
#    with regime ≡ `sedimented` (and the round-4 bottom-mass
#    criterion satisfied). The "fully-settled" entry-point.
#
...
md_lines.append(
    ...
    "Values are radii at the §5.1 regime-band edges; '—' = no cell at "
    "this (h, t_obs) reaches that band."
)
md_lines.append("")
md_lines.append(_markdown_band_table(
    ...
    grid,
    ti_room,
    title="Largest radius that stays homogeneous",
    rule=(
        "*Use this column to pick the maximum particle size that lets the "
        "suspension stay well-mixed (`c(h)/c(0) ≥ 0.95`) for the planned "
...
md_lines.append(_markdown_band_table(
    ...
    grid,
    ti_room,
    title="Smallest radius that classifies as sedimented",
    rule=(
        "*Use this column to pick the minimum particle size for "
        "sedimentation experiments. Round-4 second criterion: cells in "
        "transit are not sedimented; this is the smallest r where the "
        "bottom 5 % layer captures ≥ 95 % of the mass.*"
    ),
))
md_path.write_text("\n".join(md_lines))
...
print(_markdown_band_table(
    ...
    grid,
    ti_room,
    title="Largest homogeneous radius (preview)",
    rule="*(at 25 °C; full multi-T table in design_table_max_homogeneous_r.csv)*",
))
