...
def is_feasible(
    ...
    *,
    ...
    Returns ``(feasible, n_steps, dt)``. A cell with ``feasible = False``
    should be classified by Method A / Method C instead.
    """
    dt = adaptive_timestep(v_sed, diff, h, alpha=alpha, beta=beta)
    n_steps = max(1, int(math.ceil(t_total / dt)))
    return (n_steps <= max_steps, n_steps, dt)
...
def simulate(
    *,
    ...
    n_snapshots
        ...
        N=1e5 ensemble at 1000 steps, full-trajectory storage is ~800 MB,
